...
/* 인사제도 상단 아이콘원형 */
function rollingActive03 (activeList03) {
	$(activeList03).each(function  (index) {
		$itemList03 = $(this);
		$item03 = $itemList03.find(".personnel-icon-con");
		itemLength03 = $item03.length;
		startNum03 = 0;
		rollingSpeed03 = $itemList03.data("rolling-time");
		
		function visualTime03(){
			if(startNum03 < ( itemLength03 - 1)){
				startNum03++;
			}else{
				startNum03 = 0;
			}
		...
		function visualPlay03(){
			$item03.each(function(id){
				if(id == startNum03){
					$(this).addClass("active"); // li에 클래스 붙이기
				}else{
					$(this).removeClass("active");
		...
		visualPlay03();
		visual_timer03 = setInterval(visualTime03,rollingSpeed03);
	});
}
