...
function initHeaderGNB() {
  $(".cm-drop-menu-box-JS").each(function () {
    ...
    // 중복 방지 위해 기존 이벤트 제거
    $dropOpenBtn.off("click");
    $dropBox.off("mouseenter mouseleave");

    if (eventState === "hover") {
      $dropBox.hover(
        function () {
          $dropList.stop(true, true).slideDown(500);
          $dropBox.addClass("open");
        },
        function () {
          $dropList.stop(true, true).slideUp(500);
          $dropBox.removeClass("open");
        }
      );
    } else if (eventState === "click") {
      $dropOpenBtn.on("click", function (e) {
        e.stopPropagation();
...
// Family
function initFooterFamily() {
  const langOpen = "열기";
  const langClose = "닫기";

  $(".link_open").off("click").on("click", function () {
    const $this = $(this);
    const isOpen = $this.attr("title") === langOpen;
    $this
      .attr("title", isOpen ? langClose : langOpen)
